...
<div class="pageContent sortDrag" selector="h1" layoutH="0">
	<%
		List<ResultPerformance>  results = (List<ResultPerformance>) request.getAttribute("results");
		for(int i=0;i<results.size();i++){
			ResultPerformance result = results.get(i);
			Interface inter = new InterfaceService().findInterfaceById(result.getInterfaceId());
	%>
	
	<div class="panel collapse" minH="100">
		<h1 style="color:#FF00FF;"><%=i+1 %>、<%=inter.getInterfaceName()%>:的执行结果</h1>
		<div>
			<fieldset>
				<legend>结论</legend>
				<p>
					...
					<span style="color:blue;">开始时间为：<%=result.getBeginDate()%></span><br/>
					<span style="color:blue;">结束时间为：<%=result.getEndDate()%></span><br/>
					<span style="color:blue;">使用时间为：<%=result.getUseDate()%>毫秒</span><br/>
						<%
